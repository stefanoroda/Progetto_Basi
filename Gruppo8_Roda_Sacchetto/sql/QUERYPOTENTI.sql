...
#FINALE ma no È LA PARTE INIZIALE
SELECT UTENTE.MATRICOLA, UTENTE.NOME, UTENTE.COGNOME, CONTEGGIO.DIPARTIMENTO,CONTEGGIO.CONTEGGIO
FROM UTENTE
 JOIN (
...
SELECT UTENTE.NOME, UTENTE.COGNOME, MAX(CONTEGGIO) AS MaxConteggio
    FROM (
     ...
     FROM BOH
     GROUP BY MATRICOLA
     HAVING MAX(CONTEGGIO);
     
     SELECT * 
     FROM BOH
     ORDER BY MATRICOLA;
     
SELECT BOH.ID, BOH.MATRICOLA, BOH.NOME, BOH.COGNOME, BOH.DIPARTIMENTO, BOH.CONTEGGIO
FROM BOH
JOIN (
  SELECT MATRICOLA, MAX(CONTEGGIO) AS MAX_CONTEGGIO
  FROM BOH
  GROUP BY MATRICOLA
) t2 ON BOH.MATRICOLA = t2.MATRICOLA AND BOH.CONTEGGIO = t2.MAX_CONTEGGIO;
...
SELECT BOH.MATRICOLA, BOH.NOME, BOH.COGNOME, BOH.DIPARTIMENTO, BOH.CONTEGGIO
FROM 
(
  SELECT UTENTE.MATRICOLA, UTENTE.NOME, UTENTE.COGNOME, CONTEGGIO.DIPARTIMENTO, CONTEGGIO.CONTEGGIO
  FROM UTENTE
...
JOIN (
  SELECT MATRICOLA, MAX(CONTEGGIO) AS MAX_CONTEGGIO
  FROM (
    SELECT UTENTE.MATRICOLA, CONTEGGIO.DIPARTIMENTO, COUNT(*) AS CONTEGGIO
    FROM UTENTE
    JOIN (
      SELECT PRESTITO.MATRICOLA, SUCCURSALE.DIPARTIMENTO, COUNT(*) AS CONTEGGIO
      FROM PRESTITO
    ...
    GROUP BY UTENTE.MATRICOLA, CONTEGGIO.DIPARTIMENTO
  ) AS MAX_CONTEGGIO
  GROUP BY MATRICOLA
) t2 ON BOH.MATRICOLA = t2.MATRICOLA AND BOH.CONTEGGIO = t2.MAX_CONTEGGIO ORDER BY MATRICOLA;
...
SELECT * 
                FROM PRESTITO
                WHERE DATEDIFF(DATA_PRESTITO,2023-08-20)<30;
